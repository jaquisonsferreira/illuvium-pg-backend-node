...
export class CurrentStatsDto {
  @ApiProperty({ description: 'Total value locked (formatted)', example: '85.00M' })
  tvl: string;

  @ApiProperty({ description: 'Total value locked (raw)', example: '85000000.00' })
  tvl_raw: string;

  @ApiProperty({ description: 'Vault size in underlying asset', example: '5,312.50 ILV/ETH' })
  vault_size: string;

  @ApiProperty({ description: 'Current token price in USD', example: '1500.00' })
  token_price: string;
...
export class ChartDataPointDto {
  @ApiProperty({ description: 'Timestamp in ISO 8601 format', example: '2024-12-15T04:30:00Z' })
  timestamp: string;
...
export class VaultDetailsResponseDto {
  @ApiProperty({ description: 'Vault identifier', example: 'ILV_ETH_vault_base' })
  vault_id: string;
  ...
  @ApiProperty({ description: 'Vault name', example: 'Illuvium / Ethereum LP Vault' })
  name: string;

  @ApiProperty({ description: 'Underlying asset name', example: 'Illuvium / Ethereum LP' })
  ...
  @ApiProperty({ 
    ...
    example: 'This vault allows you to stake ILV/ETH LP tokens and earn 300 Shards / $1,000 as rewards.' 
  })
  description: string;
  ...
  @ApiProperty({ description: 'Historical statistics', type: HistoricalStatsDto })
